    ...
    <div>
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div class="flex flex-col md:flex-row gap-8">
                    <div class="flex-shrink-0">
                        <div class="relative">
                            <img
                                :src="user.avatar"
                                :alt="user.name" class="w-32 h-32 md:w-40 md:h-40 [overflow-clip-margin:unset] rounded-full object-cover shadow-lg">
                        </div>
                    </div>
                    <div class="flex-1">
                        ...
                        <div class="space-y-3 mb-6">
                            <div class="flex items-center gap-2 text-gray-600">
                                <i class="bx bx-map">
                                </i><span class="text-sm"> {{ user.location }} </span></div>
                            <div class="flex items-center gap-2 text-gray-600">
                                <i class="bx bx-calendar"></i>
                                <span class="text-sm">Joined {{ user.joined }}</span></div>
                        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <Link :href="route(`creators`, user.username)" :data="{'product': product.slug}" preserve-scroll :only="['product']" v-for="(product, idx) in products" :key="idx"
                  ...
                  <div v-if="product.images_count > 1" class="absolute top-3 right-3 bg-white/90 rounded-full px-2 py-1 flex justify-center items-center">
        ...
        <ProductModal v-if="product" :product="product" :user="user" />
    </div>
</template>
<script setup lang="ts">
import { IUserProduct, IUserProductModal, IUserProfile } from '@/types';
...
defineProps<{
    ...
    product: IUserProductModal | null
}>();
</script>