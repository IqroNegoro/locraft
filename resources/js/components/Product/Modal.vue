<template>
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
        <div class="bg-white rounded-2xl shadow-2xl max-w-2xl w-full flex flex-col md:flex-row overflow-hidden relative">
            <div class="md:w-1/2 w-full bg-gray-100 flex flex-col max-h-3/4 items-center justify-center relative">
                <button
                    v-if="product.images.length && imageIndex != 0"
                    @click="imageIndex--"
                ...
                <img
                    ...
                    :alt="product.images?.[imageIndex]?.alt_text || ''"
                    class="rounded-xl w-full h-auto object-cover max-h-max transition-all duration-300"
                />
                <button
                    v-if="product.images && imageIndex < product.images.length - 1"
                    @click="imageIndex++"
                ...
                <div v-if="product.images.length && product.images.length > 1" class="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 justify-center mt-4">
                    <button
            ...
            <span class="inline-block bg-gray-100 text-xs text-primary px-3 py-1 rounded-full font-medium mb-1">{{ product?.category }}</span>
